...
<body>
		...
		<form action="sellerLogin">
			<div class="input-group">
				<label for="email">E-mail</label> <input type="email" id="email"
					name="email" onchange="checkEmailisPresent()" required><br>
					<span id="mailResponse" ></span>
			</div>
			
			<div class="form-buttons">
				<button class="signin-btn" type="button" onclick="sendLoginOtp()" id="sendOtp" disabled> Send OTP</button>
			</div>
			...
			<div class="form-buttons">
				<input class="signin-btn" type="submit" id="login" value="Login" >
			</div> 

			<div class="form-footer">
				<p>
					Don't have an account? <a href="sellerRegistration.jsp">Register</a>
				</p>
			</div>
		</form>
	</div>
	...
	//for seller email verify
	function checkEmailisPresent() {
		var sellerMail = document.getElementById("email").value;
		var button = document.getElementById("sendOtp");
	
		const req = new XMLHttpRequest();
		
		req.open("GET", "http://localhost:8080/autospare/isSellerPresent?email="+sellerMail);
		req.send();
		req.onload = function() {
			
			let mailResponse = req.responseText.trim();
			
			document.getElementById("mailResponse").innerHTML = mailResponse;
			document.getElementById("mailResponse").className = 'invalid';
			if (mailResponse === "Account verified") {
	...
	function sendLoginOtp() {
		...
		req.open("GET", "http://localhost:8080/autospare/sendLoginOtp?email="+sellerMail);
		req.send();
		req.onload = function() {
			
			let otpResponse = req.responseText.trim();
			
			document.getElementById("otpResponse").innerHTML = otpResponse;
			document.getElementById("otpResponse").className = 'invalid';
			if (otpResponse === "OTP sent") {
				 // Disable button if email is not found
				document.getElementById("login").disabled = false;
            } else {
            	 // Enable button if email is found
